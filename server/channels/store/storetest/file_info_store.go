...
func testFileInfoGetWithOptions(t *testing.T, rctx request.CTX, ss store.Store) {
	...
	post1_1 := makePost(channelID1, userID1)
	post1_2 := makePost(channelID3, userID1)
	post2_1 := makePost(channelID2, userID2)
	post2_2 := makePost(channelID3, userID2)
	epoch := time.Date(2020, 1, 1, 1, 1, 1, 1, time.UTC)
	file1_1 := makeFile(post1_1, userID1, epoch.AddDate(0, 0, 1).Unix(), "a")
	file1_2 := makeFile(post1_2, userID1, epoch.AddDate(0, 0, 2).Unix(), "b")
	file1_3 := makeFile(&model.Post{}, userID1, epoch.AddDate(0, 0, 3).Unix(), "c")
	file2_1 := makeFile(post2_1, userID2, epoch.AddDate(0, 0, 4).Unix(), "d")
...
func testFileInfoPermanentDeleteBatch(t *testing.T, rctx request.CTX, ss store.Store) {
	...
	require.NoError(t, err)
	bookmarkFile, err := ss.FileInfo().Save(rctx, &model.FileInfo{
		PostId:    postID,
		ChannelId: channelID,
