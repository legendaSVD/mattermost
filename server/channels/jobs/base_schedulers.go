...
func (scheduler *PeriodicScheduler) NextScheduleTime(_ *model.Config, _ time.Time , _ bool /* lastSuccessfulJob */, _ *model.Job) *time.Time {
	nextTime := time.Now().Add(getRandomDelay(jitterRange)).Add(scheduler.period)
	return &nextTime
}
func (scheduler *PeriodicScheduler) ScheduleJob(rctx request.CTX, _ *model.Config , _ bool /* lastSuccessfulJob */, _ *model.Job) (*model.Job, *model.AppError) {
...
func (scheduler *DailyScheduler) NextScheduleTime(cfg *model.Config, now time.Time , _ bool /* lastSuccessfulJob */, _ *model.Job) *time.Time {
...
func (scheduler *DailyScheduler) ScheduleJob(rctx request.CTX, _ *model.Config , _ bool /* lastSuccessfulJob */, _ *model.Job) (*model.Job, *model.AppError) {
	return scheduler.jobs.CreateJob(rctx, scheduler.jobType, nil)
}
const jitterRange = 2000
