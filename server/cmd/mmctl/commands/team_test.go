...
func (s *MmctlUnitTestSuite) TestSearchTeamCmd() {
	...
	s.Run("Test search results are sorted", func() {
		...
		s.Require().Equal(mockTeam5, printer.GetLines()[0])
		s.Require().Equal(mockTeam1, printer.GetLines()[1])
		s.Require().Equal(mockTeam3, printer.GetLines()[2])
		s.Require().Equal(mockTeam4, printer.GetLines()[3])
		s.Require().Equal(mockTeam2, printer.GetLines()[4])
	})
