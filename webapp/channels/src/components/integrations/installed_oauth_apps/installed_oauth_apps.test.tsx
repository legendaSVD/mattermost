...
describe('components/integrations/InstalledOAuthApps', () => {
    ...
    test('should match snapshot', () => {
        ...
        expect(shallow(<div>{wrapper.instance().oauthApps('first')}</div>)).toMatchSnapshot();
        expect(shallow(<div>{wrapper.instance().oauthApps('ZZZ')}</div>)).toMatchSnapshot();
        expect(shallow(<div>{wrapper.instance().oauthApps()}</div>).find('Connect(InstalledOAuthApp)').length).toBe(2);
    ...
    test('should match snapshot for Apps', () => {
        ...
        props.actions.loadOAuthAppsAndProfiles = newGetOAuthApps;
        const wrapper = shallow<InstalledOAuthApps>(<InstalledOAuthApps {...props}/>);
        expect(shallow(<div>{wrapper.instance().oauthApps('second')}</div>)).toMatchSnapshot();
    });
