...
export default class AbstractList extends React.PureComponent<Props, State> {
    ...
    private renderRows = (): JSX.Element | JSX.Element[] => {
        ...
        const pageStart = this.state.page < 1 ? 0 : (this.state.page * PAGE_SIZE);
        const pageEnd = this.state.page < 1 ? PAGE_SIZE : (this.state.page + 1) * PAGE_SIZE;
        const pageData = this.props.data.slice(pageStart, pageEnd).map(this.props.renderRow);
        return pageData;
    };
