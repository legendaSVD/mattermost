...
describe('date_utils', () => {
    ...
    describe('stringToMoment', () => {
        ...
        it('should handle relative dates more efficiently without double conversion', () => {
            ...
            expect(resultTomorrow!.month()).toBe(0);
    ...
    describe('parseISO integration', () => {
        ...
        it('should reject invalid dates that parseISO catches', () => {
            expect(stringToMoment('2025-02-30')).toBeNull();
            expect(stringToMoment('2025-13-01')).toBeNull();
            expect(stringToMoment('01/15/2025')).toBeNull();
            expect(stringToMoment('invalid-date')).toBeNull();
        });
        ...
        it('should accept any valid ISO format', () => {
            expect(stringToMoment('2025-01-15')?.isValid()).toBe(true);
            expect(stringToMoment('2025-01-15T14:30Z')?.isValid()).toBe(true);
            expect(stringToMoment('2025-01-15T14:30:00')?.isValid()).toBe(true);
            expect(stringToMoment('2025-01-15T14:30:00.123Z')?.isValid()).toBe(true);
        });
        it('should be stricter than before with invalid formats', () => {
            expect(stringToMoment('2025-02-30')).toBeNull();
            expect(stringToMoment('2025-1-1')).toBeNull();
            expect(stringToMoment('25-01-15')).toBeNull();
        });
    });
    describe('momentToString', () => {
        it('should convert moment to date string', () => {
    ...
    describe('Complete Timezone Conversion Chain', () => {
        it('should handle full conversion cycle: User timezone → UTC storage → Display timezone', () => {
            ...
            expect(storedValue).toBe('2025-01-15T19:30:00Z');
            const retrievedMoment = stringToMoment(storedValue, displayTimezone);
            expect(retrievedMoment?.tz(displayTimezone).format('YYYY-MM-DD HH:mm:ss')).toBe('2025-01-15 19:30:00');
            expect(retrievedMoment?.tz(userTimezone).format('YYYY-MM-DD HH:mm:ss')).toBe('2025-01-15 14:30:00');
        });
        it('should handle timezone boundary edge cases', () => {
            const userTimezone = 'America/Los_Angeles';
            const userSelectedTime = moment.tz('2025-01-15T23:30:00', userTimezone);
            const storedValue = momentToString(userSelectedTime, true);
            expect(storedValue).toBe('2025-01-16T07:30:00Z');
        ...
        it('should handle min/max date comparisons across timezones', () => {
            ...
            expect(userSelectionDate < resolvedMinDate).toBe(true);
        });
        it('should handle cross-timezone display correctly', () => {
            const storedUTC = '2025-01-15T19:30:00Z';
            const timezones = {
                'America/New_York': '2025-01-15 14:30:00',
                'Europe/London': '2025-01-15 19:30:00',
                'Asia/Tokyo': '2025-01-16 04:30:00',
                'Australia/Sydney': '2025-01-16 06:30:00',
            };
