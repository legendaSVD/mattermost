...
describe('posts', () => {
    ...
    describe('when a channel is left (LEAVE_CHANNEL)', () => {
        ...
        it('MM-67130 should handle posts with multiple embeds, only removing affected permalinks', () => {
            ...
            expect(nextState.post1.metadata.embeds[0]).toBe(state.post1.metadata.embeds[0]);
            expect(nextState.post1.metadata.embeds[1]).toBe(state.post1.metadata.embeds[2]);
        });
    });
